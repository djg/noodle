...
use crate::{impl_comptr, opt_ptr, AsPtr};
use winapi::{
    shared::dxgi::IDXGISurface,
    um::{
        d2d1::{ID2D1Brush, ID2D1Image},
...
impl DeviceContext {
    pub fn create_bitmap_from_dxgi_surface(
        ...
        unsafe {
            ...
            let hr = self.CreateBitmapFromDxgiSurface(
                surface.as_ptr(),
                bitmap_properties,
    ...
    pub fn begin_draw(&self) {
        unsafe { self.BeginDraw() }
    }

    pub fn end_draw(&self) {
        unsafe {
            let hr = self.EndDraw(ptr::null_mut(), ptr::null_mut());
            assert!(hr == 0);
        }
    }
    ...
    pub fn create_solid_color_brush<'a>(
        ...
        unsafe {
            let mut native = SolidColorBrush::default();
            let hr = self.CreateSolidColorBrush(
                color,
    ...
    pub fn fill_ellipse(&self, ellipse: &Ellipse, brush: &impl AsPtr<ID2D1Brush>) {
        unsafe {
            self.FillEllipse(ellipse, brush.as_ptr());
        }
    }

    pub fn set_target(&self, image: &impl AsPtr<ID2D1Image>) {
        unsafe {
            (&*self.as_ptr()).SetTarget(image.as_ptr());
        }
    }
}
